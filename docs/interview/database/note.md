...
在同一个事务中，多次读取同一数据时，值可能不同。（关注点在同一个数据是否保持一致）

**3. 幻读（Phantom Read）**

一个事务在读取某一范围的数据时，另一个事务插入或删除了该范围内的数据。当前事务再次读取时，发现了“幻影”数据。（关注点在结果集的数量有没有变化）
...
| **SERIALIZABLE**     | 不可能    | 不可能       | 不可能    | 最严格隔离级别，事务按顺序执行，通过加锁防止所有并发问题。 |
...
**B+树**：对B树的进一步优化，B+树非叶子节点上是不存储数据的，仅存储键值，因此每个节点能容纳更多子节点，树的高度更低。并且所有叶子节点通过指针连接成一个有序链表。

- **精确查询**：时间复杂度为 **O(log n)**，与树的高度成正比。
- **范围查询**：B+树的叶子节点是有序链表，范围查询（比如 `BETWEEN a AND b`）可以直接顺着链表扫描，时间复杂度为 **O(k)**（k 为结果集大小）。而 B 树没有链表结构，范围扫描需要中序遍历整棵树，代价大。
...
1.阶（Fan-out）：每个节点最多有多少个子节点。阶越高，层数越少。

2.数据总量：数据量越大，B+树越高

3.每页能放多少记录：由页面大小（一般为16KB）和索引键大小决定

举例：

- 数据量：1000 万行

- 每个页能放 300 条记录（常见值，实际与索引大小相关）

- B+ 树的分支因子（即一个非叶子节点指向的子节点数）≈ 300

那我们大致需要几层呢？

第 0 层（叶子节点）：存储 1000 万条记录 → 需要约 `10000000 / 300 = 33334` 个页

第 1 层（非叶子节点）：每个节点最多指向 300 个叶子页 → `33334 / 300 ≈ 112` 个节点

 第 2 层（更上层）：`112 / 300 ≈ 1`，也就是一层可以容纳全部

所以：这棵 B+ 树只需要 **3 层**（根节点 + 中间层 + 叶子层）就能覆盖 1000 万条数据！
...
根据业务需求来设计索引：对于经常用于查询条件的字段，应该创建索引；对于范围查询，可以考虑使用复合索引。

避免创建过多的索引：索引在插入、删除和更新操作时需要同时更新索引，这增加了开销。

复合索引顺序：如果创建复合索引，需要考虑索引键的顺序。一般来说，应该将选择性高的字段放在前面，这样可以更快地过滤掉不需要的数据。

**（9）双字段索引单个字段查能否命中？**

查询单个字段时是否能命中索引，取决于 索引的最左前缀原则。

查询第一个字段能使用索引，查询第二个字段不能。

**（10）为什么不是所有索引的叶子节点都存放数据**

InnoDB 索引结构分两种：

- 聚簇索引：叶子节点存储完整行记录。每个表只能有一个聚簇索引，通常就是**主键索引**。

- 非聚簇索引：叶子节点只存索引列+主键值，要整行再去主键索引中查。

- 为了节省空间和提升维护效率，只存主键值，真正数据由主键索引维护。

**（11）索引越多越好吗？**

索引虽然能提高查询效率，但是每次`INSERT` / `UPDATE` / `DELETE` 都需要**更新索引**，索引越多，写操作越慢。

索引本质是维护的一棵数据结构（如 B+ 树），需要占用更多存储空间。

**（12）分库分表的机会**

分库的依据：一般按业务线分库，比如用户库、订单库、支付库。

分表的依据：按时间（常用于日志、订单）、按ID取模、按地区（`user_beijing`, `user_shanghai`）。
...
①添加合适的索引，可以显著提高查询性能。当多个字段一起参与查询时，复合索引比单列索引更高效。同时要避免索引失效（对索引字段使用函数、使用 LIKE '%keyword%'（前置 % 会导致索引失效）、使用 OR 连接多个字段，部分字段无索引）。

②避免 SELECT * 取出所有字段，只查询必要字段。如果查询数据量大，可以分页。

③分库分表优化：通过按某些列（如日期、地理区域等）分区，将大表拆分为多个较小的物理表，从而减少查询时的扫描范围，提高查询性能。
...
MVCC（Multi-Version Concurrency Control）即多版本并发控制，主要用于提高数据库的并发性能。

MVCC 的本质是：**通过为每个事务提供数据的快照（Snapshot）来实现并发控制**，从而避免加锁，提高数据库的并发性能。

每次读取数据时，不是去读当前数据的“最新值”，而是读该事务**可见的历史版本**。

每条数据记录都有多个版本，每个版本对应一个写入它的事务时间戳或事务 ID。

**好处**

1.读写分离：读操作不加锁，可以和写操作并发执行，大大减少阻塞与锁竞争。比如：一个事务正在写数据，另一个事务可以照样读老版本数据，不会被阻塞。

2.避免脏读和不可重复读。同一事务中，每次查询读取到的是同一个“快照”，保证了数据一致性（Repeatable Read）。

**底层实现**

1.每行数据都有两个隐藏字段：trx_id（最后一次修改它的事务ID）、roll_pointer(指向旧版本记录的undo log)

2.当前事务通过对比trx_id与自己的事务ID，判断某条记录是否“可见”

### 9. 延时双删是怎么做的，是否能保证强一致性
...
### 12. 数据库分片sharding的概念，它有什么优势和挑战?

数据库分片是指把一个数据库中的数据按照一定的规则划分成多个数据子集（分片），并将这些分片分别存储在不同的物理服务器或存储节点上。常见的有范围分片、哈希分片、列表分片等。

- **范围分片**：按照数据的某个范围字段（如用户 ID、时间范围等）将数据划分到不同的分片。例如，将用户 ID 从 1 - 1000 的数据存储在分片 1，1001 - 2000 的数据存储在分片 2 等。
- **哈希分片**：对数据的某个关键字进行哈希运算，根据哈希结果将数据分配到不同的分片。比如，对用户的手机号码进行哈希，根据哈希值将用户数据分配到不同的分片。
- **列表分片**：根据预先定义的列表规则将数据划分到不同的分片。例如，将不同地区的用户数据分别存储在不同的分片，如北京用户数据在一个分片，上海用户数据在另一个分片。

**优势**

**扩展性**：数据库分片允许通过添加更多的服务器来增加系统的存储容量和处理能力，而不是单纯依靠提升单个服务器的硬件性能（垂直扩展）。

**高可用性**：如果某个分片节点出现故障，其他分片节点仍然可以正常工作，不会影响整个数据库系统的可用性。

**负载均衡**：分片技术可以将负载均匀地分布到各个节点上，避免单个节点因负载过高而出现性能瓶颈或故障。这样可以提高整个系统的稳定性和可靠性。

### 13. MySQL日志，主从复制

错误日志（Error Log）：记录 MySQL 启动、运行、关闭过程中的错误信息。

查询日志（General Log）：记录所有的 SQL 请求（语句级）。

慢查询日志（Slow Query Log）：记录执行时间超过阈值的 SQL 语句。

二进制日志（Binary Log）：记录所有对数据库有更改的操作（用于主从复制、数据恢复）。

中继日志（Relay Log）：从库接收到主库 binlog 后生成的本地日志。

事务日志（InnoDB Redo/Undo Log）：InnoDB 引擎的事务提交、回滚、崩溃恢复依赖日志。

**主从复制原理**

- 所有对主库的数据更改操作（INSERT、UPDATE、DELETE）都会记录到 **binary log（二进制日志）** 中。

- 从库通过一个 **IO线程** 与主库连接，并拉取主库的 binlog 内容，写入本地的 **relay log（中继日志）**。

- 从库通过 **SQL线程** 读取 relay log 内容并执行，从而达到主库数据同步。

### 14. 数据库崩溃如何恢复

**InnoDB 自动崩溃恢复机制**

- 数据库服务意外中止（如断电、kill、OOM），但数据文件仍在。

- mysql会自动根据`redo log` 回放未写入的数据

**通过备份 + Binlog 恢复**

- 如果你做过备份 + 开启了 binlog，可以做到“恢复到任意时间点”。

- 通过备份的sql文件，恢复基础数据。通过binlog恢复数据。

**主从架构下的恢复**

- 情况：主库崩溃，但从库完整

- 临时将从库提升为主库，从从库导出数据，恢复主库
