...
### 18. volatile关键字的主要作用，以及它为什么线程不安全？
...
- **轻量级锁（Lightweight Locking）**：
  - 当多个线程尝试竞争锁时，升级为轻量级锁。
  - 轻量级锁使用 CAS（Compare-And-Swap）操作实现线程安全，而不涉及操作系统的线程调度。
- **重量级锁（Heavyweight Locking）**：
  - 当锁竞争激烈时，升级为重量级锁，依赖操作系统的 Mutex（互斥锁）实现。
...
### 32. 使用hashmap时，如果只重写了equals没有重写hashcode会出现什么问题
...
### 38.线程池怎么创建，线程池可以用来干嘛，你用过的是哪种线程池
...
CAS（Compare And Swap，比较并交换）是一种广泛应用于并发编程中的原子操作，尤其是在无锁编程中，它通过硬件指令实现了一种高效的线程安全操作。

CAS 的核心是将**内存中的某个值与预期值进行比较**，如果相等，则将其更新为新值；否则，不做任何修改。整个操作是原子的，意味着整个操作的过程不能被中断或打断，其他线程无法在 CAS 操作的过程中干预。因此，CAS 能确保在多线程环境中，操作某个共享变量时不会发生数据竞争。

应用场景

- CAS 是实现无锁队列、无锁栈等并发数据结构的基础。

- Java 的 `ThreadPoolExecutor` 使用 CAS 来管理线程状态。
