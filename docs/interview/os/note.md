...
### 1.4 死锁发生的原因。如何解决死锁问题？

**死锁**：死锁是指多个线程/进程**互相等待资源释放，形成永久阻塞**的状态。  
...
- 互斥：资源一次只能被一个线程占有。
- 持有并等待：线程已持有某些资源，同时又申请新的资源。
- 不可剥夺：已占有的资源不能被强制抢走，只能主动释放。
- 循环等待：存在一个资源循环等待链（如 A 等 B，B 等 A）。

**如何避免？**

1.统一资源获取顺序（破坏循环等待），所有线程按照同样的顺序申请资源（例如先加 A，再加 B），防止循环等待。

2.避免长时间持有锁（减少等待，避免占有并等待），及时释放不再使用的资源
